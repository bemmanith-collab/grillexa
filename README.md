...
**Settling a backlog in one go: `backend/scripts/bulk-settle.js`.** Hundreds of old consignments nobody settled are still "awaiting settlement" forever, and the Settle screen does one at a time. The script settles every outstanding consignment delivered *strictly before* a day, all remaining quantity as sold or all as returned, through the same `applySettlement` the route uses — one transaction per consignment, so each comes out exactly as a hand settlement would, ledger and Sale and Return rows included. `--as` has no default because sold recognises revenue and returned does not, and that is nobody's call but the owner's. Dry run by default; it prints the count, units, value and per-store breakdown, and `--apply` writes. `--verbose` lists every consignment rather than a progress line every fiftieth — in a dry run it turns the summary into an actual preview of the rows about to change, and on `--apply` it leaves an audit trail of which ones were touched. Totals alone answer "how much", never "which ones". `--date` defaults to **today**, not to the cutoff, matching what the Settle screen does. It used to default to `--before`, which quietly future-dated every Sale and ledger row whenever the cutoff was a future day — a backlog cleared "up to next month" landed its revenue next month. Pass `--date` explicitly only when the settlement genuinely belongs on another day. It has to be run on the Fly machine, where the database is:
...
**It hunts for a 5m lock and stops the instant it has one.** Under 5m there is nothing left to wait for — that is hardware GPS under open sky — so the watch is cleared on sight and the chip powers down. A merely good reading (20m, 65m) does **not** end the watch: the loop keeps listening and keeps the best reading seen, because a capture that settled early on a "good" fix returned a pin the server rejects as too coarse for billing, after the person had already walked off. It ends on a perfect fix, on 8s of silence (a phone on a weak network stops delivering readings without ever calling the error handler — the stall guard re-arms on every reading and is the only thing that would notice), or at the 25s ceiling, whichever comes first. The constants are at the top of `frontend/src/lib/locate.js`; `frontend/test/locate.js` checks all three exits against a scripted stream.

**The timeout fallback keeps the pin but not the address.** When the ceiling passes without a 5m lock, the best reading seen is saved as the pin — a 20m pin is still far better than none — but the reverse-geocode step is skipped: `captureFix` returns `locked: false`, and the page will not name a street from a point that is only *probably* on the right road, because that address would look authoritative and nobody would check it. The message says what happened and what to do (step into the open, or type the address). The watch is also cancellable: the Stores page hands it an `AbortSignal` and aborts on unmount and on a second tap, so leaving the page mid-capture powers the GPS down instead of leaving a 25s watch running for a promise nobody is waiting on. A permission revoked mid-stream after a usable reading is treated as the fallback arriving early, not as an error.
...
The watcher itself is **shared, not re-written**: `frontend/src/lib/locate.js` holds the `watchPosition` loop that was tuned on the Stores page — keep the most accurate reading, clear the watch the instant it is under 5m, otherwise hunt until silence or the 25s ceiling — and both callers use it. A plain `getCurrentPosition` somewhere else in the app would have quietly undone all of that tuning, since the first fix a phone returns is usually the cheap one.
...
The page refreshes every 5 minutes, but only while it is actually on screen and recently touched — the app sits open in a pocket all day, and waking it to refetch a screen nobody is reading costs battery, data and (see the Neon notes) database compute hours. Returning to the foreground refreshes immediately. Pull-to-refresh is implemented in the page: installed as an app there is no browser chrome to pull against, and that is the gesture everyone tries first.
...
**It remembers which day it has already dealt with.** Those five-minute ticks each ran three queries to re-derive a decision that cannot change again today — and that poll, not the app's actual traffic, is what exhausted the free tier's compute hours on 2026-09-26. Once the day is settled (sent, already written, nothing to suggest) the check returns before touching the database, so the compute gets to sleep. The cache is in memory and deliberately *not* a lock: the unique constraint on `WhatsAppReminder.sentFor` is still the only thing standing between two machines and two notifications, and a restart just re-checks the day once. The test that matters is the rollover — a cache that never lets go means the reminder silently stops arriving.
...
**Polled, not socketed.** `fly.toml` sets `auto_start_machines = true`, so more than one machine can be awake — a socket opened against one would never see a message posted to the other, the same problem the WhatsApp reminder solves with a unique constraint rather than a flag in memory. The room polls `GET /api/team-chat?after=<id>` every five seconds while the tab is visible **and somebody is actually there**, which is an indexed lookup returning nothing most of the time, and the existing web push covers the app being closed. The second condition is `isIdle()` from `frontend/src/lib/userIdle.js`: visibility alone covers the phone in a pocket but not the tablet left open on a counter, and twelve queries a minute at an empty room keeps the database awake — and billable — all day. Five minutes without a tap, key, scroll or focus and every poll in the app stops; one touch brings them all back. A socket layer would need shared pub/sub before it beat this.
...
Database is **Neon** (Launch plan, `ap-southeast-1` Singapore), reached over the public internet from Fly `sin`. Measured from the app machine: **3ms warm, 432ms on the first query after idle** — Neon suspends the compute after ~5 minutes of no queries, so the first action of the morning pays that wake-up once. The `/health` endpoint deliberately does **not** touch the database; if it did, the health check every 15s would keep the compute awake permanently.

**Neon bills by the hour the compute is awake, not by the query, and that is the whole cost model.** Anything that touches the database more often than once every five minutes holds it open indefinitely — the query itself is free, the wall-clock hour it prevents from sleeping is not. A poll on a one-minute timer and a poll on a five-second timer cost exactly the same: all of it.

This was learned the hard way on **2026-09-26**, when the free tier's 100 monthly compute-hours ran out mid-month and the database began refusing every connection. The whole app went down — login included, since checking a password is a read. The cause was not traffic: it was the WhatsApp reminder's five-minute timer running from 07:00 to midnight IST to send one notification, holding the compute awake 17 hours a day for ~128 compute-hours a month of nothing. Fixed by the day cache below and the idle gate in `frontend/src/lib/userIdle.js`; the plan moved to Launch the same day so the shops could trade while it was sorted out.
